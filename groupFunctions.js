...
function jsonToHtmlPreset(dataJson){
  ...
  reply += '<h2>Client(s)</h2><div class="Client" data-max="6"><div class="addFieldset icons" onclick="addFieldSet(this)">+</div>';
  var text="";
  for (var i = 0; i<dataJson.Client.length; i++){
    text += '<fieldset class="preset"><legend>'+dataJson.Client[i].itemName+'</legend><div class="logoUploadContainer"><div class="imgContainer"><img src="'+dataJson.Client[i].logoUrl+'"></div><p>Upload clients\'s logo</p><p>*180 px or bigger is recommended</p><p class="photoErr" style="color:red; visibility: hidden; font-size: 12px">Please choose some logo</p><input type="file" class="hidden" name="logo" accept=".jpg, .jpeg, .png"><div class="chooseFileBtn" onclick="uploadPhoto(this)">Choose File</div></div><div class="textBlock"><input type="text" name="itemname" val="" placeholder="'+dataJson.Client[i].itemName+'" value="'+dataJson.Client[i].itemName+'"><textarea rows="14" name="descr">'+dataJson.Client[i].descr+'</textarea></div><div class="removeFieldSet icons" onclick="deleteFieldSet(this)">-</div></fieldset>';
  }
  reply+=text + '</div>';
  reply += '<h2>Project(s)</h2><div class="Project" data-max="6"><div class="addFieldset icons" onclick="addFieldSet(this)">+</div>';
  text="";
  for (var i = 0; i<dataJson.Project.length; i++){
    text += '<fieldset class="preset"><legend>'+dataJson.Project[i].itemName+'</legend><div class="logoUploadContainer"><div class="imgContainer"><img src="'+dataJson.Project[i].logoUrl+'"></div><p>Upload project\'s logo</p><p>*180 px or bigger is recommended</p><p class="photoErr" style="color:red; visibility: hidden; font-size: 12px">Please choose some logo</p><input type="file" class="hidden" name="logo" accept=".jpg, .jpeg, .png"><div class="chooseFileBtn" onclick="uploadPhoto(this)">Choose File</div></div><div class="textBlock"><input type="text" name="itemname" val="" placeholder="'+dataJson.Project[i].itemName+'" value="'+dataJson.Project[i].itemName+'"><textarea rows="14" name="descr">'+dataJson.Project[i].descr+'</textarea></div><div class="removeFieldSet icons" onclick="deleteFieldSet(this)">-</div></fieldset>';
  }
  reply+=text + '</div></div>';
  return reply;
}
