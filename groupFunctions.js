...
function addFieldSet(elem) {
  ...
  if(x$(type).children('.preset').length>=limit){
    ...
    type.append('<fieldset class="preset"><legend>'+name+'</legend><div class="logoUploadContainer"><div class="imgContainer"><img src="'+imgPlaceHolderUrl+'"></div><p>Upload '+name.toLowerCase()+'\'s logo.</p><p>*180 px or bigger is recommended</p><p class="photoErr" style="color:red; visibility: hidden; font-size: 12px">Please choose some logo</p><input type="file" class="hidden" name="logo" accept=".jpg, .jpeg, .png"><div class="chooseFileBtn" onclick="uploadPhoto(this)">Choose File</div></div><div class="textBlock"><input type="text" name="itemname" val="" placeholder="'+name+' name"><textarea rows="14" name="descr" placeholder="Description"></textarea></div><div class="removeFieldSet icons" onclick="deleteFieldSet(this)">-</div></fieldset>');
  }
}
...
function getToSendData() {
  var promiseObj = new Promise(function (resolve, reject){
    ...
    x$('fieldset').each(function(){
      ...
      if(itemName===""){return;}
      var descr = x$(this).find('textarea[name="descr"]').val();
...
function jsonToHtmlPreset(dataJson){
  ...
  for (var i = 0; i<dataJson.Client.length; i++){
    text = '<fieldset class="preset"><legend>'+dataJson.Client[i].itemName+'</legend><div class="logoUploadContainer"><div class="imgContainer"><img src="'+dataJson.Client[i].logoUrl+'"></div><p>Upload clients\'s logo</p><p>*180 px or bigger is recommended</p><p class="photoErr" style="color:red; visibility: hidden; font-size: 12px">Please choose some logo</p><input type="file" class="hidden" name="logo" accept=".jpg, .jpeg, .png"><div class="chooseFileBtn" onclick="uploadPhoto(this)">Choose File</div></div><div class="textBlock"><input type="text" name="itemname" val="" placeholder="'+dataJson.Client[i].itemName+' value="'+dataJson.Client[i].itemName+'"><textarea rows="14" name="descr" placeholder="'+dataJson.Client[i].descr+' value="'+dataJson.Client[i].Description+'"></textarea></div><div class="removeFieldSet icons" onclick="deleteFieldSet(this)">-</div></fieldset>';
  }
  reply+=text + '</div>';
  ...
  for (var i = 0; i<dataJson.Project.length; i++){
    text = '<fieldset class="preset"><legend>'+dataJson.Project[i].itemName+'</legend><div class="logoUploadContainer"><div class="imgContainer"><img src="'+dataJson.Project[i].logoUrl+'"></div><p>Upload project\'s logo</p><p>*180 px or bigger is recommended</p><p class="photoErr" style="color:red; visibility: hidden; font-size: 12px">Please choose some logo</p><input type="file" class="hidden" name="logo" accept=".jpg, .jpeg, .png"><div class="chooseFileBtn" onclick="uploadPhoto(this)">Choose File</div></div><div class="textBlock"><input type="text" name="itemname" val="" placeholder="'+dataJson.Project[i].itemName+' value="'+dataJson.Project[i].itemName+'"><textarea rows="14" name="descr" placeholder="'+dataJson.Project[i].descr+' value="'+dataJson.Project[i].descr+'"></textarea></div><div class="removeFieldSet icons" onclick="deleteFieldSet(this)">-</div></fieldset>';
  }
  reply+=text + '</div></div>';
  return reply;
}
...
function firstStart(){
  x$(x$('.span8')[0]).append('<div class="columnGroup"><div class="form-field"><label for="number">Contact Number</label><input type="text"  class="input-full" name="number" value=""></div><div class="form-field"><label for="number">Company URL</label><input type="text"  class="input-full"  name="url" value=""></div><div class="form-field"><label for="turnover">Turn Over</label><select name="turnover"><option value="Identification">Less than 1</option><option value="Identification" selected>1 - 1.1</option><option value="Evaluation">1.1 - 1.2</option><option value="Ongoing">1.2 - 1.3</option><option value="Supervision">1.3 - 1.4</option><option value="Complited">More than 1.4</option></select></div><div class="form-field"><label for="turnover">Codes & Normes Certification</label><input type="text"  class="input-full"  name="codes" value=""></div><div class="form-field"><label for="turnover">Association membership</label><input type="text"  class="input-full"  name="assoc" value=""></div></div>');
