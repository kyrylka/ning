function init(){
    ...
    x$('.activityFeed-counts').each(function(){
        ...
        if(x$(curGreenContainer).find('.activityFeed-commentFormDisplay').length!=0){
            ...
            elem.replaceWith('<div class="activityComments" onclick="console.log(this);let par = x$(this).parent().parent();  par.find(\'.status-commentForm\').show(); par.find(\'.activityFeed-comments\').show(); par.find(\'.activityComments\').remove()"><span class="number"></span><img src="'+controlsList.comments+'"><span class="text">Comments</span></div>')
        }else{
