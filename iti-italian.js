...
function addResultsToFront(){  
  window.myInterval=setInterval(function() {
    if ((typeof results != "undefined" && results!="No results")||(results != null && results!="No results")){
      ...
      var resOnPage = $('.searchResult').length;
...
if($('#searchLoader').length!=0){
    ...
    if($('.searchResult').length === resPP_end){
      loaderActions("remove");
      if($('#searchMore').length==0){
        moreActions("add");
      }
...
function errorMessage(str){
  ...
  if (str === "RenumFailed500"){
    ...
    $('#searchRequestErorrMessage').remove();
    $('body').append('<div class="messageContainer" id="searchRequestErorrMessage"><div class="message"><div id="advancedSearchClose">X</div><p style="color:red"><strong>Error!</strong></p><p>'+str+'</p><div class="reNum" onclick="reNum2()">Continue updating</div></div></div>');
     $('#advancedSearchClose').on('click', function(){
      $('#searchRequestErorrMessage').remove();      
        loaderActions("remove");      
    });    
  }else {
...
loaderActions("remove");
    ...
    $('#searchRequestErorrMessage').remove();
    $('body').append('<div class="messageContainer" id="searchRequestErorrMessage"><div class="message"><div id="advancedSearchClose">X</div><p style="color:red"><strong>Info</strong></p><p>'+str+'</p></div></div>');
     $('#advancedSearchClose').on('click', function(){
      $('#searchRequestErorrMessage').remove();
loaderActions("remove");
    });    
    ...
    str = "The numbers have been updated, you can close this page";
    $('body').append('<div class="messageContainer" id="searchRequestErorrMessage"><div class="message"><div id="advancedSearchClose">X</div><p style="color:red"><strong>Error!</strong></p><p>'+str+'</p></div></div>');
     $('#advancedSearchClose').on('click', function(){
      $('#searchRequestErorrMessage').remove();
loaderActions("remove");
    });
...
function loaderActions(str){
  if(str==="remove"){
    ...
    $('#searchResults').append('<div id="searchLoader">Loading...</div>');
  }else{
    errorMessage('action to perform with loader is not recognised');
  }
}
function appendResLoc(resLocal, i){
  var photo, locaLoc, spec, htmlCode;
  if(resLocal[i].hasOwnProperty('photo')===false){
    photo='https://static.ning.com/iti-italian-network/widgets/profiles/gfx/defaults/profile-dark.png';
  }else{
  ...
  spec="Not specified";
  htmlCode="";
  for(var j=0; j<resLocal[i].profileQuestions.length; j++){
    if(resLocal[i].profileQuestions[j].questionId==="q10"){
      spec=resLocal[i].profileQuestions[j].answer.answer;
    }
    if(resLocal[i].profileQuestions[j].questionId==="q11"){
...
function getProfileQuestions(blackList, dependencies){
  ...
  catRequest.open("GET","https://external.ningapis.com/xn/rest/iti-italian-network/2.0/Network?xn_pretty=true&ID=7113166&fields=profileQuestions");
  ...
  catRequest.onreadystatechange = function(){
    if(catRequest.readyState === 4 && catRequest.status === 200){
      ...
      var formToPost='<div class="advancedSearchQuestion"><label class="advansedSearchLabel" for="q">Find a Translator</label><select class="advansedSearchSelect" id="typeOfMember"><option selected="selected" value="Not specified">Both</option><option>Translator</option><option>Interpreter</option></select></div><div class="advancedSearchQuestion"><label class="advansedSearchLabel" for="q">Working from (language)</label><select class="advansedSearchSelect" id="fromLanguage" "><option selected="selected" value="Not specified">Please select</option><option value="Albanian">Albanian</option><option value="Croatian">Croatian</option><option value="Dutch">Dutch</option><option value="English">English</option><option value="French">French</option><option value="German">German</option><option value="Italian">Italian</option><option value="Maltese">Maltese</option><option value="Polish">Polish</option><option value="Portuguese">Portuguese</option><option value="Russian">Russian</option><option value="Spanish">Spanish</option></select></div><div class="advancedSearchQuestion"><label class="advansedSearchLabel" for="q">into (language)</label><select class="advansedSearchSelect" id="toLanguage" "><option selected="selected" value="Not specified">Please select</option><option value="Albanian">Albanian</option><option value="Croatian">Croatian</option><option value="Dutch">Dutch</option><option value="English">English</option><option value="French">French</option><option value="German">German</option><option value="Italian">Italian</option><option value="Maltese">Maltese</option><option value="Polish">Polish</option><option value="Portuguese">Portuguese</option><option value="Russian">Russian</option><option value="Spanish">Spanish</option></select></div>';
      var questionList=[];      
...
function searchOnClick(){
  ...
  var q2searchArray=[];
  var locationArr = [];
  var valToSearch=$('#typeOfMember').val();
